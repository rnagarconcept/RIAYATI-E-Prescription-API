...
namespace DataAccess
{
    public class OracleDataAccessRepository : OracleRepositoryBase
    {
        ...
        public PhysicianLoginResponseModel GetPhysicianLogin(string facilityLic, string phyLic)
        {
            var output = new PhysicianLoginResponseModel();
            OracleConnection con = null;
            using (con = OpenConnection())
            {
                var OraCmd = new OracleCommand();
                ...
                OraCmd.CommandType = CommandType.StoredProcedure;
                OraCmd.Parameters.Add("P_FACILITY_LIC", OracleDbType.NVarchar2, facilityLic, ParameterDirection.Input);
                OraCmd.Parameters.Add("P_PHY_LIC", OracleDbType.NVarchar2, phyLic, ParameterDirection.Input);
                OraCmd.Parameters.Add(new OracleParameter("p_refcur", OracleDbType.RefCursor, ParameterDirection.Output));
                if (con.State == ConnectionState.Closed)
                {
                    con.Open();
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        output.C_LIC = reader.ToString("C_LIC");
                        output.C_USER = reader.ToString("C_USER");
                        output.C_PWD = reader.ToString("C_PWD");
                        output.F_USER = reader.ToString("F_USER");
                        output.F_PWD = reader.ToString("F_PWD");
                    }
                }
            }
            return output;
        }
        public FacilityLoginResponseModel GetFacilityDetails(string facilityLic)
        {
            var output = new FacilityLoginResponseModel();
            OracleConnection con = null;
            using (con = OpenConnection())
            {
                var OraCmd = new OracleCommand();
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {   
                        output.F_LIC = reader.ToString("F_LIC");
                        output.F_USER = reader.ToString("F_USER");
                        output.F_PWD = reader.ToString("F_PWD");                        
                    }
                }
        ...
        public List<OrderingPhyResponseModel> GetOrderingPhy(int eRxNo)
        {
            var output = new List<OrderingPhyResponseModel>();
            OracleConnection con = null;
            using (con = OpenConnection())
            {
                var OraCmd = new OracleCommand();
                OraCmd.Connection = con;
                OraCmd.CommandText = $"SELECT DISTINCT ERX_NO,CLINICIAN_ID FROM  PAT_ERX_ENCOUNTER WHERE ERX_NO=:P_ERX_NO";
                OraCmd.CommandType = CommandType.StoredProcedure;
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        obj.ERX_NO = reader.ToString("ERX_NO");
                        obj.CLINICIAN_ID = reader.ToString("CLINICIAN_ID");
                        output.Add(obj);
                    }
                }
            }
